...
This criterion is linked to a Learning Outcome Tree Menu Semantics
When navigating through the tree menu with a keyboard, elements are announced with tree menu semantics.

This criterion is linked to a Learning Outcome Tree Submenus
...
(function($, window, document, undefined) {
                ...
                $me.attr({
                    ...
                    'aria-posinset':
                        $me
                            .parent()
                            .children()
                            .index($me) + 1 // define position of the current element on the current level
                });

                $($me.contents()[0]).wrap('<span></span>'); // wrap text element of each treitem with span element

                if ($me.children('ul').length) {
                    // if the current treeitem has submenu

                    if (plugin.options.expandAll) {
                        // expand or collapse all tree levels based on configuration
                        $me.attr({
                            'aria-expanded': true
                        });
                    } else {
    ...
    Plugin.prototype.selectItem = function($item, plugin) {
        ...
        $elem
            .find('[aria-selected=true]') // remove previous selection
            .attr({
                tabindex: -1,
                'aria-selected': false
            });
        ...
        $elem
            .find('.focused') // remove highlight form previousely selected treeitem
            .removeClass('focused');

        $elem.find('li').attr({
            // remove all treeitems from tab order
            tabindex: -1
        });
    ...
    Plugin.prototype.onMouseOver = function(event) {
        ...
        $me
            .children('span') // add highlight to currently selected treeitem
            .addClass('mouseover');
    };
